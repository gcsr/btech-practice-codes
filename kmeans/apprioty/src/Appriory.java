...
public class Appriory {
	...
	public static void main(String[] gcs){
		...
		while(true){
			//System.out.println("\n option 1: create data \n option 2: display transactions\n option 3: run appriory\n option 4: run improvd appriory\n option 5: graphs\n other : exit");
			System.out.println("\n option 1: create data \n option 2: display transactions\n option 3: run appriory\n option 4: graphs\n other : exit");
			int option=scanner.nextInt();
			
			
			switch(option){
				case 1: appriory.createTransactions();
						break;
				case 2: appriory.display();
						break;
				case 3: appriory.runAppriory();
						break;
						
				/*case 4: appriory.runAppriory();
						break;*/
						
				case 4: appriory.createTransactions();
						break;
	...
	private void findJoin(){
		...
		while(leftIt.hasNext()){
			...
			//System.out.println("displaying new set");
			//display(newSet);
			...
			while(rightIt.hasNext()){
				...
				for(int i=0;i<currentLength;i++){
					...
					//System.out.println("after break before new is displaying new set");
					//display(newSet);
					int[] newIds=new int[nextLength];
					...
					//System.out.println(" before new is displaying new set");
					//display(newSet);
					...
					//System.out.println("displaying new set");
					//display(newSet);
					//System.out.println(newAppr.tr);
					//if(!checkEquality(newSet,newAppr))
					newSet.add(newAppr);
	...
	/*private void display(List<ApprioryTransaction> list){
		
		int ssssize=list.size();	
		
		for(int i=0;i<ssssize;i++){
			System.out.println(list.get(i).tr);
			
		}
		
	}*/
	
	private boolean checkEquality(List<ApprioryTransaction> list,ApprioryTransaction one){
		
		System.out.println("check equality called");
		int ssssize=list.size();
		
		for(int i=0;i<ssssize;i++){
			System.out.println(list.get(i).tr);
			System.out.println(one.tr);
			if(list.get(i).tr.isSame(one.tr))
				return true;
		}
		return false;
	}
	...
	private void displayFiltered(){
		...
		/*Transaction[] dis=new Transaction[filtered.size()];
		for(int i=0;i<dis.length;i++){
			dis[i]=filtered.get(i).tr;
		}*/
		...
		/*System.out.println("in display Filtered");
		Iterator iterator=filtered.iterator();
		
		while(iterator.hasNext()){
			ApprioryTransaction rft=(ApprioryTransaction)iterator.next();
			System.out.println(rft.tr);
			System.out.println(rft.count);
		}*/
	}
