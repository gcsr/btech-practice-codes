...
public class MultilayerPerceptron 
  ...
  private double learnForPrinting;
  private double momentumForPrinting;
  ...
  private void updateNetworkWeights(double l, double m) {
	//toString();
	  learnForPrinting=l;
	  momentumForPrinting=m;
    for (int noc = 0; noc < m_numClasses; noc++) {
      //update weights
      m_outputs[noc].updateWeights(l, m);
    }
    //System.out.println("update networ weights called");
  ...
  public void buildClassifier(Instances i) throws Exception {
    ...
    System.out.println("initial weghts");
    //System.out.println(MultilayerPerceptron.this);
  ...
  public String toString() {
    ...
    for (int noa = 0; noa < m_neuralNodes.length; noa++) {
      ...
      System.out.println("learn is "+learnForPrinting);
      System.out.println("momentum "+momentumForPrinting);
      System.out.println(con.errorValue(false));
      NeuralConnection[] inputsHere = con.getInputs();
      double[] cWeights = con.getChangeInWeights();
      double[] weightsHere = con.getWeights();
      
      System.out.println("before change in weights and weights");
      for(int ippp=0;ippp<cWeights.length;ippp++){
    	  System.out.println(cWeights[ippp]+"   "+weightsHere[ippp]+"\t");
      }
      double learnTimesError = 0;
      learnTimesError = learnForPrinting * con.errorValue(false);
      System.out.println("learnTimesError="+learnTimesError);
      
      
      double c = learnTimesError + momentumForPrinting * cWeights[0];
      weightsHere[0] += c;
      System.out.println("threshold change=learnTimesError + momentumForPrinting * cWeights[0]");
      System.out.println("threshold change="+c);
      System.out.println("threshold changed valuee="+weightsHere[0]);
      cWeights[0] = c;
      
      int stopValue = con.getNumInputs() + 1;
      System.out.println("individual input change=learnTimesError * inputsHere[noaaaa-1].outputValue(false) and  c += momentumForPrinting * cWeights[noaaaa];");
      for (int noaaaa = 1; noaaaa < stopValue; noaaaa++) {
        
        c = learnTimesError * inputsHere[noaaaa-1].outputValue(false);
        c += momentumForPrinting * cWeights[noaaaa];
        System.out.println("change for input "+noaaaa+" "+c);
        weightsHere[noaaaa] += c;
        cWeights[noaaaa] = c; 
      }
      
      System.out.println("after change in weights and weights");
