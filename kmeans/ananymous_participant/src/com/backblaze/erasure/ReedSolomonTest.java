...
public class ReedSolomonTest {
    ...
    public void testBigEncodeDecode() {
        ...
        for (i=0;i<dataShards.length;i++) {
            //for (int i = 0; i < shard.length; i++) {
                dataShards[i] = ("bava 32534"+i+"bava").getBytes();
                //System.out.println(dataShards[i].toString());
    ...
    public String getErasureCoding(String msg){
    	final Random random = new Random(0);
        final int dataCount =1;
        final int parityCount = 1;
        final int shardSize = msg.length()*8;
        byte [] [] dataShards = new byte [dataCount] [shardSize];
        
        for (int i=0;i<dataShards.length;i++) {
            //for (int i = 0; i < shard.length; i++) {
                dataShards[i] = msg.substring(i, i+1).getBytes();
                //System.out.println(dataShards[i].toString());
                //i++;//(byte) random.nextInt(256);
            //}
        }
        ...
        return new String(allShards[0]);

    }
    
    public String getOriginalMessage(String msg){
    	final Random random = new Random(0);
        final int dataCount =1;
        final int parityCount = 1;
        final int shardSize = msg.length()*8;
        ...
        final int totalCount = dataCount + parityCount;
        final int shardLength = msg.length()*8/2;//dataShards[0].length;

        // Make the list of data and parity shards.
       // assertEquals(dataCount, dataShards.length);
        final int dataLength = shardLength;//dataShards[0].length;
        byte [] [] allShards = new byte [totalCount] [];
     ...
     /*   for (int numberMissing = 0; numberMissing < parityCount + 1; numberMissing++) {
        ...
        int length=testShards.length;
        
        for(int i=0;i<dataCount;i++){
        	System.out.println(new String(allShards[i]));
        	System.out.println(new String(testShards[i]));
        }
    ...
    public static void main(String[] gcs){
    	...
    	ReedSolomonTest test=new ReedSolomonTest();
    	String message="";
    	//dataCount=message.
    	//ReedSolomon codec = ReedSolomon.create(dataCount, parityCount);
    	test.testBigEncodeDecode();
    }
}
