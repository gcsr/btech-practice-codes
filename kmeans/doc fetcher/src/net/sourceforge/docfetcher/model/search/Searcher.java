...
public final class Searcher {
	...
	public List<ResultDocument> search(@NotNull String queryString)
		...
		try {
			...
			try{
				String test1= JOptionPane.showInputDialog("Please input paragraphsize(left,right character size): ");
				leftRight=Integer.parseInt(test1);
			}catch(Exception ex){
				leftRight=200;
			}
			...
			String absolutePath;
			//int i=0;
			PdfSearcher pr=new PdfSearcher();
			ArrayList<DocResult> searchResults=new ArrayList<DocResult>();
			int kfc=0;
			for(ResultDocument dc:results){
				
			
				if(dc.isPdfFile()){
					
					ResultParser rp=new ResultParser(leftRight);
					
					/*kfc++;
					PDDocument pddDocument =PDDocument.load(dc.getPath().getCanonicalPath());
					...
					HighlightedString hls=HighlightService.highlight(query, isPhraseQuery,textStripper.getText(pddDocument));
					System.out.println(hls.getRangeCount());
					
					//System.out.println(textStripper.getText(pddDocument));
					//searchResults.addAll(pr.print(queryString,dc.getPath().getCanonicalPath()));		
					//break;*/
					System.out.println("calling add al");
					searchResults.addAll(rp.print(dc, query, isPhraseQuery));
				}
			}
			
			WriteResultsToDoc.createAndUpdate(searchResults);
