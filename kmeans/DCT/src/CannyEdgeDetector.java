...
public class CannyEdgeDetector {
	...
	public void setSourceImage(int[] pixels){
		if (sourceImage == null) {
			sourceImage = new BufferedImage(width, height, BufferedImage.TYPE_3BYTE_BGR);
		}
		sourceImage.getWritableTile(0, 0).setDataElements(0, 0, width, height, pixels);
	}
	
	public int[] getImagePixels(){
		System.out.println("image pixels length "+imagePixels.length);
		return imagePixels;
	}
	...
	private void readLuminance() {
		...
		if (type == BufferedImage.TYPE_INT_RGB || type == BufferedImage.TYPE_INT_ARGB) {
			System.out.println("image type is RGB or ARGB");
			...
			System.out.println("image type is USHORT_GRAY");
			short[] pixels = (short[]) sourceImage.getData().getDataElements(0, 0, width, height, null);
			for (int i = 0; i < picsize; i++) {
				data[i] = (pixels[i] & 0xffff) / 256;
			}
		} else if (type == BufferedImage.TYPE_3BYTE_BGR) {
			System.out.println("image type is BGR");
            byte[] pixels = (byte[]) sourceImage.getData().getDataElements(0, 0, width, height, null);
