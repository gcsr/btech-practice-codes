...
public class worm1
{
	...
	public worm1(int pwidth,int pheight,obstacles1 obts)
	{
		obs=obts;
	...
	public boolean touchedat(int x,int y)
	{
		...
		while(i!=headposn)
		{
			if((Math.abs(cells[i].x+RADIUS-x)<=RADIUS)&&(Math.abs(cells[i].y+RADIUS-y)<=RADIUS))
			return true;
			i=(i+1)%MAX_POINTS;
	...
	public void draw(Graphics gc)
	{
		if(npoints>0)
		{
			...
			gc.fillOval(cells[headposn].x,cells[headposn].y,12,12);
		}
	}
}